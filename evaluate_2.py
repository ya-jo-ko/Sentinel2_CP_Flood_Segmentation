...
# Load best temperatures (optional)
if args.temperature_scaling == 1:
    with open('./saved_models/S1GFloodstemps_K20_ep20_2025-07-09.pkl', 'rb') as f:
...
def k_cv_cp_test(test_dataset, test_labels, models, validation_scores, alpha=0.1, nc_score='res', ts=0):
    ...
    for label_idx, label in enumerate(candidate_labels):
        ...
        #test_av = np.median(test_nc_scores, axis=0)
        test_av = np.max(test_nc_scores, axis=0)
        print(test_av.shape)
