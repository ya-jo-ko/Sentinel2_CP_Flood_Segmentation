...
if args.dataset == 'S1GFloods':
    ...
    path1 = args.path
    path2 = args.path2 if args.model_type == 'S1&S2' else path1
...
# ------------------------- Training Function -------------------------
def train_k_fold(file_splits, alpha, K, batch_size, epochs, ts, nc_score='res'):
    models, val_scores = [], []
...
# ------------------------- Save Results -------------------------
def save_models(models, scores, save_dir, K, epochs, alpha):
    os.makedirs(save_dir, exist_ok=True)
    date = datetime.now().strftime("%Y-%m-%d")
    with open(os.path.join(save_dir, f"models_K{K}_ep{epochs}_a{alpha}_{date}.pkl"), 'wb') as f:
        pickle.dump(models, f)
    with open(os.path.join(save_dir, f"scores_K{K}_ep{epochs}_a{alpha}_{date}.pkl"), 'wb') as f:
...
models, val_scores = train_k_fold(
    splits,
    alpha=args.alpha,
    K=args.K,
    batch_size=8,
...
save_models(models, val_scores, args.save_dir, args.K, args.epochs, args.alpha)
